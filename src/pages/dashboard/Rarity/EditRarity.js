...
export default function EditDom() {
  ...
  const defaultValues = useMemo(
    () => ({
      name:   currentrarity?.name || '',
      description:  currentrarity?.desc ||  '',
    }),
    []
  );
  ...
  const OnSubmit = async () => {
    ...
    try {
      const rarity=new URLSearchParams();
      rarity.append('name',formValues?.name)
      rarity.append('desc',formValues?.description)
   
   await axios.put(`rarity/${id}`,rarity)
